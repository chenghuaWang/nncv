...
#define GEN_PASS_DEF_TENSORTOVECTORVECTORIZEPAD
...
/// for attributes.
static Value getAsIndexValue(OpFoldResult attrOrValue, OpBuilder& builder, Location loc) {
  IntegerAttr attr;
...
/// ```
struct VectorizePadWithConditions final : public OpRewritePattern<tensor::PadOp> {
  ...
  LogicalResult matchAndRewrite(tensor::PadOp padOp, PatternRewriter& rewriter) const override {
    ...
    if (!matchPattern(paddingValue, m_Constant(&paddingAttr))) { return failure(); }
    ...
    for (int i = 0; i < tensorRank; ++i) {
      ...
      auto srcDimSize = rewriter.createOrFold<tensor::DimOp>(loc, padOp.getSource(), i);
      auto lb = getAsIndexValue(lowPads[i], rewriter, loc);
      auto ub = rewriter.create<affine::AffineApplyOp>(loc, addMap, ValueRange{lb, srcDimSize});
      paddedDimLBs[i] = lb;
      paddedDimUBs[i] = ub;
    }

    Type elementType = padOp.getType().getElementType();
    auto fullVectorType = VectorType::get(dropLeadingOne(paddedTensorShape), elementType);
    ...
    auto sliceVectorType = VectorType::get(dropLeadingOne(sliceVectorShape), elementType);
    ...
    for (int i = 0; i < totalCount; ++i) {
      ...
      for (int dim : llvm::reverse(paddedDimIndices)) {
        ...
        valueIndices[dim] = rewriter.createOrFold<arith::ConstantIndexOp>(loc, staticIndices[dim]);
      ...
      for (int dim : paddedDimIndices) {
        Value lt = rewriter.createOrFold<arith::CmpIOp>(loc, arith::CmpIPredicate::sge,
                                                        valueIndices[dim], paddedDimLBs[dim]);
        Value ge = rewriter.createOrFold<arith::CmpIOp>(loc, arith::CmpIPredicate::slt,
                                                        valueIndices[dim], paddedDimUBs[dim]);
        Value logicalAnd = rewriter.createOrFold<arith::AndIOp>(loc, lt, ge);
        condition = rewriter.createOrFold<arith::AndIOp>(loc, condition, logicalAnd);
      }
      ...
      fullVector = rewriter.create<vector::InsertStridedSliceOp>(
          ...
          llvm::ArrayRef(staticIndices).take_back(fullVectorType.getRank()), staticStrides);
    }

    Value fullTensor =
        rewriter.create<tensor::EmptyOp>(loc, paddedTensorShape, elementType, ValueRange());
    valueIndices.assign(tensorRank, zeroIndex);
    rewriter.replaceOpWithNewOp<vector::TransferWriteOp>(padOp, fullVector, fullTensor,
                                                         valueIndices);
...
struct TensorToVectorVectorizePadPass
    : public ::impl::TensorToVectorVectorizePadBase<TensorToVectorVectorizePadPass> {
...
void populateVectorizePadPatterns(RewritePatternSet& patterns, PatternBenefit baseBenefit) {
