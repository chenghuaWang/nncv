...
namespace mlir::nncv {
#define GEN_PASS_DEF_PREPAREMODERNVECGPU
...
class PrepareModernVecGpuPass final
    : public impl::PrepareModernVecGpuBase<PrepareModernVecGpuPass> {
...
std::unique_ptr<InterfacePass<mlir::FunctionOpInterface>> createPrepareModernVecGpuPass() {
  return std::make_unique<PrepareModernVecGpuPass>();
}
