...
bool vectorizeMatMul(IRRewriter& rewriter, mlir::Operation* op) {
  if (failed(linalg::vectorize(rewriter, op))) { return false; }
  return true;
}
...
class AdvancedVecDispatchedPass final
    : public impl::AdvancedVecDispatchedBase<AdvancedVecDispatchedPass> {
...
std::unique_ptr<InterfacePass<mlir::FunctionOpInterface>> createAdvancedVecDispatchedPass() {
  return std::make_unique<AdvancedVecDispatchedPass>();
}
